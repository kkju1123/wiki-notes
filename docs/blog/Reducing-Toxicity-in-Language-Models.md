...
tags: []
summary: ''
fetched_at: '2026-09-20T08:45:32.413447+00:00'
---
...
![Image 1](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/offensive-taxonomy.png)
...
![Image 2](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/TS-bootstrap.png)
...
1.   _Build it_: A BERT model is trained to classify toxic comments on the [Jigsaw dataset](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/#jigsaw).
...
![Image 3](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/build-break-fix.png)
...
![Image 4](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/build-break-fix-it-results.png)

The comparison of performance on standard and adversarial tasks of models trained on standard () and adversarial data collection (). The subscript indicates the number of training rounds. (Image source: [Dinan et al. 2019](https://arxiv.org/abs/1908.06083))
...
![Image 5](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/about-perspective-api.png)
...
![Image 6](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/unprompted-toxicity.png)
...
[](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/)Despite of its popularity, Perspective API contains known biases, as summarized by [Gehman et al. (2020)](https://arxiv.org/abs/2009.11462):
...
![Image 7](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/self-diagnosis-toxicity-score.png)
...
Given an input prompt , a textual description of undesired attributes , and the language model , self-debiasing computes the difference between the probability of next words without and with the self-debiasing template :

Because is expected to boost the probabilities of undesired words, should be negative for undesirable words.

In self-diasing decoding, a scaling function of the probability difference is used to alter the true sampling distribution,

In the paper, they used a soft variant where the probabilities of the words with negative are reduced w.r.t. the magnitude of :

![Image 8](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/self-debiasing-decoding.png)
...
![Image 9](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/offensive-text-style-transfer.png)
...
Let be the desired style ( for offensive and for non-offensive), and be the -th sample of style , . Both the encoder and decoder take a sample (or hidden state) along with a style label. The classifier predicts a probability distribution over the style labels given an input sample.

Following the illustration in Fig. 9:

*   The top branch of forward transfer is auto encoder: ​. Two losses are computed: 
    *   Reconstruction loss measures how well the decoder can reconstruct the sample back:

*   The bottom branch of forward transfer: 
...
Let and be two mutually exclusive style variables and is a sample of style , style transformer computes several losses:
...
![Image 11](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/style-transformer-training.png)
...
Let be the attribute of (e.g. “civil”) and be the other opposite attribute (e.g. “toxic”). These two attributes are mutually exclusive. The goal is to learn a mapping function such that it translates to a new fluent sequence with target attribute while preserving ’s content.

The encoder-decoder model is trained with the loss:

*   The denoising auto-encoder loss is the loss for denoising auto-encoders, where is a [masking](https://lilianweng.github.io/posts/2019-01-31-lm/#pre-training-tasks) function same as in BERT training:

*   The cycle consistency loss ([Zhu et al. 2017](https://arxiv.org/abs/1703.10593)) has to produce a non-differentiable pseudo-prediction and it does not take gradient backpropagation.
...
![Image 12](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/RealToxicityPrompts-experiments.png)
...
![Image 13](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/safe-chatbot-system.png)
...
*   _Gender bias mitigation_: 
    ...
    *   Precisely, given a gendered word list, tag the training samples with , , , and labels, indicating whether the response contains female / male words ( contains, does not contain). At test time, the system runs with a control label to avoid outputting gender specific words.
...
[](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/)**Jigsaw Toxic** Comments Classification Dataset (2018): contains about 160k examples extracted from Wikipedia discussion pages, each annotated for 7 classes: toxic, severe toxic, obscene, threat, insult, identity hate and non-toxic. The labelling process involved 5000 crowdsourced annotators. [[Download](https://www.kaggle.com/c/jigsaw-toxic-comment-classification-challenge)]
...
[](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/)**OLID** (Offensive Language Identification Dataset; 2019): contains 14,100 English tweets, annotated according to the three-level taxonomy as described [here](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/#categorization-of-toxic-content). [[Download](https://sites.google.com/site/offensevalsharedtask/olid)]

[](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/)**SOLID** (Semi-Supervised Offensive Language Identification Dataset; 2020): contains 9+ Millions tweets annotated following OLID’s three level taxonomy. [[Download](https://sites.google.com/site/offensevalsharedtask/solid)]

[](https://lilianweng.github.io/posts/2021-03-21-lm-toxicity/)**RealToxicityPrompts** dataset (2020): contains 100k sentence snippets from the web with Perspective API toxicity scores for studying the risk of neural toxic degeneration in language models. [[Download](https://allenai.org/data/real-toxicity-prompts)]
