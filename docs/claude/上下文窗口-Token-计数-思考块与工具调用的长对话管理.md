---
title: 上下文窗口：Token 计数、思考块与工具调用的长对话管理
...
- Prompt Caching
- Agent
summary: 讲解 Claude 上下文窗口的组成与计数规则，包括思考令牌、工具调用，以及长对话如何压缩管理。
fetched_at: '2026-09-20T02:57:11.170163+00:00'
---
...
The following diagram illustrates the standard context window behavior for API requests 1:

![Image 1: Diagram of turns accumulating in the context window until the conversation approaches the token limit](https://platform.claude.com/docs/images/context-window.svg)

_1 Chat interfaces such as [claude.ai](https://claude.ai/) can also manage the context window on a rolling "first in, first out" basis._
...
![Image 2: Diagram of thinking on a model that strips previous thinking blocks: each turn's thinking block is generated in the output and not carried into later turns' input](https://platform.claude.com/docs/images/context-window-thinking.svg)
...
![Image 3: Diagram of thinking with tool use: thinking is kept with its tool result, then dropped on the next user turn on models that strip previous thinking blocks](https://platform.claude.com/docs/images/context-window-thinking-tools.svg)
