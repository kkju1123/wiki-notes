...
tags: []
summary: ''
fetched_at: '2026-09-20T08:48:25.011076+00:00'
---
...
| Definition | The rate of change of a function $f \left(\right. x , y , z , \ldots \left.\right)$ at a point $\left(\right. x_{0} , y_{0} , z_{0} , \ldots \left.\right)$, which is the slope of the tangent line at the point. | The instantaneous rate of change of $f \left(\right. x , y , z , \ldots \left.\right)$ in the direction of an unit vector $\overset{\rightarrow}{u}$. | It points in the direction of the greatest rate of increase of the function, containing all the partial derivative information of a multivariable function. |
...
The term is the partial derivative on the x-direction, which is computed as the color difference between the adjacent pixels on the left and right of the target, f(x+1, y) - f(x-1, y). Similarly, the term is the partial derivative on the y-direction, measured as f(x, y+1) - f(x, y-1), the color difference between the adjacent pixels above and below the target.

There are two important attributes of an image gradient:

*   **Magnitude** is the L2-norm of the vector, .
*   **Direction** is the arctangent of the ratio between the partial derivatives on two directions, .

![Image 1](https://lilianweng.github.io/posts/2017-10-29-object-recognition-part-1/image-gradient-vector-pixel-location.png)
...
Repeating the gradient computation process for every pixel iteratively is too slow. Instead, it can be well translated into applying a convolution operator on the entire image matrix, labeled as using one of the specially designed convolutional kernels.

Let’s start with the x-direction of the example in Fig 1. using the kernel sliding over the x-axis; is the convolution operator:
...
![Image 2](https://lilianweng.github.io/posts/2017-10-29-object-recognition-part-1/manu-2004.png)
...
![Image 3](https://lilianweng.github.io/posts/2017-10-29-object-recognition-part-1/manu-2004-sobel-operator.png)

Apply Sobel operator kernel on the example image.

You might notice that most area is in gray. Because the difference between two pixel is between -255 and 255 and we need to convert them back to [0, 255] for the display purpose. A simple linear transformation ( + 255)/2 would interpret all the zeros (i.e., constant colored background shows no change in gradient) as 125 (shown as gray).
...
![Image 4](https://lilianweng.github.io/posts/2017-10-29-object-recognition-part-1/HOG-histogram-creation.png)
...
![Image 5](https://lilianweng.github.io/posts/2017-10-29-object-recognition-part-1/block_histogram.png)
...
Say, we use a undirected graph to represent an input image. One vertex represents one pixel. One edge connects two vertices and . Its associated weight measures the dissimilarity between and . The dissimilarity can be quantified in dimensions like color, location, intensity, etc. The higher the weight, the less similar two pixels are. A segmentation solution is a partition of into multiple connected components, . Intuitively similar pixels should belong to the same components while dissimilar ones are assigned to different components.
...
*   **Internal difference**: , where is the minimum spanning tree of the components. A component can still remain connected even when we have removed all the edges with weights < .
*   **Difference between two components**: . if there is no edge in-between.
*   **Minimum internal difference**: , where helps make sure we have a meaningful threshold for the difference between components. With a higher , it is more likely to result in larger components.

The quality of a segmentation is assessed by a pairwise region comparison predicate defined for given two regions and :
...
2.   Initially, each pixel stays in its own component, so we start with components.
3.   Repeat for : 
    ...
    *   If and belong to two different components and as in the segmentation , we want to merge them into one if ; otherwise do nothing.
...
![Image 6](https://lilianweng.github.io/posts/2017-10-29-object-recognition-part-1/image-segmentation-indoor.png)
...
![Image 7](https://lilianweng.github.io/posts/2017-10-29-object-recognition-part-1/manu-2013.png)
...
![Image 8](https://lilianweng.github.io/posts/2017-10-29-object-recognition-part-1/manu-2013-segmentation.png)
...
![Image 9](https://lilianweng.github.io/posts/2017-10-29-object-recognition-part-1/selective-search-algorithm.png)

The detailed algorithm of Selective Search.

## Configuration Variations

Given two regions , selective search proposed four complementary similarity measures:
