---
title: DeepSeek 思考模式：思维链返回、多轮回传与工具调用规则
...
- 工具调用
- API设计
summary: DeepSeek 思考模式在最终回答前输出 reasoning_content，可用 effort 控制强度；工具调用必须回传思维链，普通多轮无需回传。
fetched_at: '2026-09-17T02:54:24.444036+00:00'
---
...
*   若请求**携带 `tools` 参数**：历史轮次的 `reasoning_content` 均应回传给 API，并会被拼接进上下文。详见[工具调用](https://api-docs.deepseek.com/zh-cn/guides/thinking_mode#tool-calls)。
*   若请求**未携带 `tools` 参数**：`reasoning_content` 无需回传；即使传入 API，也会被忽略，不会拼接进上下文。详见[多轮对话拼接](https://api-docs.deepseek.com/zh-cn/guides/thinking_mode#%E5%A4%9A%E8%BD%AE%E5%AF%B9%E8%AF%9D%E6%8B%BC%E6%8E%A5)。
...
## 工具调用[​](https://api-docs.deepseek.com/zh-cn/guides/thinking_mode#tool-calls "工具调用的直接链接")
