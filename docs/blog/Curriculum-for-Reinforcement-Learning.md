...
tags: []
summary: ''
fetched_at: '2026-09-20T08:46:16.355595+00:00'
---
...
![Image 1](https://lilianweng.github.io/posts/2020-01-29-curriculum-rl/types-of-curriculum-2.png)
...
![Image 2](https://lilianweng.github.io/posts/2020-01-29-curriculum-rl/curriculum-by-transfer-learning.png)
...
[](https://lilianweng.github.io/posts/2020-01-29-curriculum-rl/)The idea of _Automatic Curriculum Learning_ was proposed by [Graves, et al. 2017](https://arxiv.org/abs/1704.03003) slightly earlier. It considers a -task curriculum as an [-armed bandit](https://lilianweng.github.io/posts/2018-01-23-multi-armed-bandit/) problem and an adaptive policy which learns to optimize the returns from this bandit.
...
*   The unobserved is the full state of the student model.
*   The observed are a list of scores for tasks.
*   The action is to pick on subtask.
*   The reward per step is the score delta. (i.e., equivalent to maximizing the score of all tasks at the end of the episode).
...
What if the task space is continuous? [Portelas, et al. (2019)](https://arxiv.org/abs/1910.07224) studied a continuous teacher-student framework, where the teacher has to sample parameters from continuous task space to generate a learning curriculum. Given a newly sampled parameter , the absolute learning progress (short for ALP) is measured as , where is the episodic reward associated with and is the reward associated with . Here, is a previous sampled parameter closest to in the task space, which can be retrieved by nearest neighbor. Note that how this ALP score is different from learning signals in [TSCL](https://lilianweng.github.io/posts/2020-01-29-curriculum-rl/#TSCL) or [Grave, et al. 2017](https://lilianweng.github.io/posts/2020-01-29-curriculum-rl/#grave-et-al-2017) above: ALP score measures the reward difference between two tasks rather than performance at two time steps of the same task.

On top of the task parameter space, a Gaussian mixture model is trained to fit the distribution of over . ε-greedy is used when sampling the tasks: with some probability, sampling a random task; otherwise sampling proportionally to ALP score from the GMM model.

![Image 4](https://lilianweng.github.io/posts/2020-01-29-curriculum-rl/ALP-GMM-algorithm.png)
...
![Image 5](https://lilianweng.github.io/posts/2020-01-29-curriculum-rl/self-play-maze.png)
...
*   In the _self-play episode_, Alice alters the state from to and then Bob is asked to return the environment to its original state to get an internal reward.
*   In the _target task episode_, Bob receives an external reward if he visits the target flag.

Note that since B has to repeat the actions between the same pair of of A, this framework only works in reversible or resettable environments.

Alice should learn to push Bob out of his comfort zone, but not give him impossible tasks. Bob’s reward is set as and Alice’s reward is , where is the total time for B to complete the task, is the time until Alice performs the STOP action and is a scalar constant to rescale the reward to be comparable with the external task reward. If B fails a task, . Both policies are goal-conditioned. The losses imply:
...
Here is the expected return, also equivalent to the success probability. Given sampled trajectories from the current policy, as long as any state belongs to the goal set, the return will be positive.
...
*   Here and can be interpreted as a minimum and maximum probability of reaching a goal over T time-steps.
...
*   Generator : produces a new goal. => expected to be a goal uniformly sampled from set.
*   Discriminator : evaluates whether a goal can be achieved. => expected to tell whether a goal is from set.

The Goal GAN is constructed similar to LSGAN (Least-Squared GAN; [Mao et al., (2017)](https://arxiv.org/abs/1611.04076)), which has better stability of learning compared to vanilla GAN. According to LSGAN, we should minimize the following losses for and respectively:

where is the label for fake data, for real data, and is the value that wants to believe for fake data. In LSGAN paper’s experiments, they used .

The Goal GAN introduces an extra binary flag indicating whether a goal is real () or fake () so that the model can use negative samples for training:

![Image 6](https://lilianweng.github.io/posts/2020-01-29-curriculum-rl/generative-goal-learning-algorithm.png)
...
*   **Solver**/Policy : In each episode, the solver gets a goal at the beginning and get a single binary reward at the end.
*   **Judge**/Discriminator : A classifier to predict the binary reward (whether goal can be achieved or not); precisely it outputs the logit of a probability of achieving the given goal, , where is the sigmoid function.
*   **Setter**/Generator : The goal setter takes as input a desired feasibility score and generates , where the latent variable is sampled by . The goal generator is designed to reversible, so can map backwards from a goal to a latent 
...
1.   Goal **validity**: The proposed goal should be achievable by an expert policy. The corresponding generative loss is designed to increase the likelihood of generating goals that the solver policy has achieved before (like in [HER](https://arxiv.org/abs/1707.01495)). 
    *    is the negative log-likelihood of generated goals that have been solved by the solver in the past.
...
1.   Goal **feasibility**: The proposed goal should be achievable by the current policy; that is, the level of difficulty should be appropriate. 
    *    is the output probability by the judge model on the generated goal should match the desired .
...
![Image 7](https://lilianweng.github.io/posts/2020-01-29-curriculum-rl/setter-judge-goal-generation.png)
...
Let be a latent task variable and be a variational distribution of , which could be a mixture model with discrete or a VAE with continuous . A variational posterior works like a classifier, predicting a skill given a state, and we would like to maximize to discriminate between data produced by different skills as much as possible. In E-step, is fitted to a set of trajectories produced by .

Precisely, given a trajectory , we would like to find such that

A simplifying assumption is made here to ignore the order of states in one trajectory.

(2) **M-Step**: This is the stage for doing meta-RL training with . The learned skill space is considered as a training task distribution. CARML is agnostic to the type of meta-RL algorithm for policy parameter updates.

Given a trajectory , it makes sense for the policy to maximize the mutual information between and , , because:

*   maximizing => diversity in the policy data space; expected to be large.
*   minimizing => given a certain skill, the behavior should be restricted; expected to be small.

Then we have,

We can set the reward as , as shown in the red part in the equation above. In order to balance between task-specific exploration (as in red below) and latent skill matching (as in blue below) , a parameter is added. Each realization of induces a reward function (remember that reward + CMP => MDP) as follows:

![Image 9](https://lilianweng.github.io/posts/2020-01-29-curriculum-rl/CARML-algorithm.png)

The algorithm of CARML. (Image source: [Jabri, et al 2019](https://arxiv.org/abs/1912.04226))

Learning a latent skill space can be done in different ways, such as in [Hausman, et al. 2018](https://openreview.net/forum?id=rk07ZXZRb). The goal of their approach is to learn a task-conditioned policy, , where is from a discrete list of tasks, . However, rather than learning separate solutions, one per task, it would be nice to learn a latent skill space so that each task could be represented in a distribution over skills and thus skills are _reused between tasks_. The policy is defined as , where and are policy and embedding networks to learn, respectively. If is discrete, i.e. drawn from a set of skills, then the policy becomes a mixture of sub-policies. The policy training uses [SAC](http://127.0.0.1:4000/lil-log/2018/04/07/policy-gradient-algorithms.html#sac) and the dependency on is introduced in the entropy term.
...
1.   It starts with a single column containing layers of neurons, in which the corresponding activation layers are labelled as . We first train this single-column network for one task to convergence, achieving parameter config .

2.   Once switch to the next task, we need to add a new column to adapt to the new context while freezing to lock down the learned skills from the previous task. The new column has activation layers labelled as , and parameters .

3.   Step 2 can be repeated with every new task. The -th layer activation in the -th column depends on the previous activation layers in all the existing columns:

where is the weight matrix of the layer in the column ; are the weight matrices for projecting the layer of the column to the layer of column (). The above weights matrices should be learned. is a non-linear activation function by choice.

![Image 10](https://lilianweng.github.io/posts/2020-01-29-curriculum-rl/progressive-networks.png)
...
[Czarnecki, et al. (2018)](https://arxiv.org/abs/1806.01780) proposed another RL training framework, **Mix & Match** (short for **M&M**) to provide curriculum through coping knowledge between agents. Given a sequence of agents from simple to complex, , each parameterized with some shared weights (e.g. by shared some lower common layers). M&M trains a mixture of agents, but only the final performance of the most complex one matters.

In the meantime, M&M learns a categorical distribution with [pmf](https://en.wikipedia.org/wiki/Probability_mass_function) probability to pick which policy to use at a given time. The mixed M&M policy is a simple weighted sum: . Curriculum learning is realized by dynamically adjusting , from to . The tuning of can be manual or through [population-based training](https://lilianweng.github.io/posts/2019-09-05-evolution-strategies/#hyperparameter-tuning-pbt).

To encourage cooperation rather than competition among policies, besides the RL loss , another [distillation](https://arxiv.org/abs/1511.06295)-like loss is added. The knowledge transfer loss measures the KL divergence between two policies, for . It encourages complex agents to match the simpler ones early on. The final loss is .

![Image 11](https://lilianweng.github.io/posts/2020-01-29-curriculum-rl/mix-and-match.png)
