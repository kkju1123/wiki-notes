---
title: Claude Thinking 机制：配置、摘要读取与多轮工具调用
...
- Token 成本
- 工具调用
summary: 理解 Claude Thinking 的开启、读取、签名回传与成本控制，掌握自适应推理机制。
fetched_at: '2026-09-20T02:54:27.696725+00:00'
---
...
![Image 1: Diagram of thinking on a model that strips previous thinking blocks: each turn's thinking block is generated in the output and not carried into later turns' input](https://platform.claude.com/docs/images/context-window-thinking.svg)
...
![Image 2: Diagram of thinking with tool use on a model that strips previous thinking blocks: thinking is kept with its tool result, then dropped on the next user turn](https://platform.claude.com/docs/images/context-window-thinking-tools.svg)
