...
tags: []
summary: ''
fetched_at: '2026-09-20T08:47:42.348162+00:00'
---
...
![Image 1](https://lilianweng.github.io/posts/2018-05-05-drl-implementation/pacman-original.gif)
...
![Image 2](https://lilianweng.github.io/posts/2018-05-05-drl-implementation/dqn-tensorboard-graph.png)
...
If we look into the standard form of the Q value target, , it is easy to notice that we use to select the best next action at state s’ and then apply the action value predicted by the same . This two-step reinforcing procedure could potentially lead to overestimation of an (already) overestimated value, further leading to training instability. The solution proposed by double Q-learning ([Hasselt, 2010](http://papers.nips.cc/paper/3964-double-q-learning.pdf)) is to decouple the action selection and action value estimation by using two Q networks, and : when is being updated, decides the best next action, and vice versa.
...
![Image 3](https://lilianweng.github.io/posts/2018-05-05-drl-implementation/tf_gather.png)

(Image source: [tf.gather() docs](https://lilianweng.github.io/posts/2018-05-05-drl-implementation/))
...
The dueling Q-network ([Wang et al., 2016](https://arxiv.org/pdf/1511.06581.pdf)) is equipped with an enhanced network architecture: the output layer branches out into two heads, one for predicting state value, V, and the other for [advantage](https://lilianweng.github.io/posts/2018-02-19-rl-overview/#value-function), A. The Q-value is then reconstructed, .

To make sure the estimated advantage values sum up to zero, , we deduct the mean value from the prediction.
...
![Image 4](https://lilianweng.github.io/posts/2018-05-05-drl-implementation/dueling-q-network.png)
...
1.   Initialize the actor network, and the critic, 
2.   Collect a new transition (s, a, r, s’): Sample the action for the current state s, and get the reward r and the next state s'.
3.   Compute the TD target during episode rollout, and TD error, .
4.   Update the critic network by minimizing the critic loss: .
5.   Update the actor network by minimizing the actor loss: .
...
![Image 5](https://lilianweng.github.io/posts/2018-05-05-drl-implementation/actor-critic-tensorboard-graph.png)
## References
