...
tags: []
summary: ''
fetched_at: '2026-09-20T08:48:57.385194+00:00'
---
...
Given a feature vector and a class label , the probability of this data point belonging to this class is:

The Naive Bayes classifier is then defined as:

Because the model has learned the prior during the training, the contribution of an individual feature value can be easily measured by the posterior, .
...
Let’s say we need to generate an explanation for a classification model . Given a data point which consists of individual values of attribute , , and is labeled with class . The _prediction difference_ is quantified by computing the difference between the model predicted probabilities with or without knowing :
...
**Problem**: If the target model outputs a probability, then great, getting is straightforward. Otherwise, the model prediction has to run through an appropriate post-modeling calibration to translate the prediction score into probabilities. This calibration layer is another piece of complication.

**Another problem**: If we generate by replacing with a missing value (like `None`, `NaN`, etc.), we have to rely on the model’s internal mechanism for missing value imputation. A model which replaces these missing cases with the median should have output very different from a model which imputes a special placeholder. One solution as presented in the paper is to replace with all possible values of this feature and then sum up the prediction weighted by how likely each value shows in the data:

Where is the probability of getting label if we replace the feature with value in the feature vector of . There are unique values of in the training set.
...
Let’s say, we have a [Bayes Classifier](https://en.wikipedia.org/wiki/Bayes_classifier) which is trained on the data set and outputs probabilities over the class labels , . And one class label is drawn from the class label pool, . This Bayes classifier is constructed as:

The _local explanation vector_ is defined as the derivative of the probability prediction function at the test point . A large entry in this vector highlights a feature with a big influence on the model decision; A positive sign indicates that increasing the feature would lower the probability of assigned to .

However, this approach requires the model output to be a probability (similar to the [“Prediction Decomposition”](https://lilianweng.github.io/posts/2017-08-01-interpretation/#prediction-decomposition) method above). What if the original model (labelled as ) is not calibrated to yield probabilities? As suggested by the paper, we can approximate by another classifier in a form that resembles the Bayes classifier :
...
Where is the index set containing the indices of data points assigned to class by the model , . is a kernel function. Gaussian kernel is a popular one among [many candidates](https://en.wikipedia.org/wiki/Kernel_(statistics)#Kernel_functions_in_common_use).

(2) Then, apply the Bayes’ rule to approximate the probability for all classes:

(3) The final estimated Bayes classifier takes the form:

Noted that we can generate the labeled data with the original model , as much as we want, not restricted by the size of the training data. The hyperparameter is selected to optimize the chances of to achieve high fidelity.

![Image 2](https://lilianweng.github.io/posts/2017-08-01-interpretation/interpretability_local_gradient.png)
...
Same as above, let us label the black-box model as . LIME presents the following steps:

(1) Convert the dataset into interpretable data representation: .
...
![Image 3](https://lilianweng.github.io/posts/2017-08-01-interpretation/LIME_interpretable_representation.png)
...
(2) Given a prediction with the corresponding interpretable data representation , let us sample instances around by drawing nonzero elements of uniformly at random where the number of such draws is also uniformly sampled. This process generates a perturbed sample which contains a fraction of nonzero elements of .

Then we recover back into the original input and get a prediction score by the target model.

Use many such sampled data points and their model predictions, we can learn an explanation model (such as in a form as simple as a regression) with local fidelity. The sampled data points are weighted differently based on how close they are to . The paper used a lasso regression with preprocessing to select top most significant features beforehand, named “K-LASSO”.

![Image 4](https://lilianweng.github.io/posts/2017-08-01-interpretation/LIME_illustration.png)

The pink and blue areas are two classes predicted by the black-box model . the big red cross is the point to be explained and other smaller crosses (predicted as pink by ) and dots (predicted as blue by ) are sampled data points. Even though the model can be very complicated, we are still able to learn a local explanation model as simple as the grey dash line. (Image source: [homes.cs.washington.edu/~marcotcr/blog/lime](https://homes.cs.washington.edu/~marcotcr/blog/lime/))
...
![Image 5](https://lilianweng.github.io/posts/2017-08-01-interpretation/LIME.png)
...
![Image 6](https://lilianweng.github.io/posts/2017-08-01-interpretation/BETA.png)

Measures for desiderata of a good model explanation: fidelity, unambiguity, and interpretability. Given the target model is , its explanation is a two level decision set containing a set of rules , where and are conjunctions of predicates of the form (feature, operator, value) and is a class label. Check [the paper](https://arxiv.org/abs/1707.01154) for more details. (Image source: [arxiv.org/abs/1707.01154](https://arxiv.org/abs/1707.01154))
...
![Image 7](https://lilianweng.github.io/posts/2017-08-01-interpretation/adversarial_examples.png)
